...
export default function App() {
  ...
  const filtered = leads.filter(l => {
    ...
    return matchSearch &&
      (filterSales === "الكل" || l.sales === filterSales) &&
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.body}>
        ...
        {tab === "add" && (
          <div style={S.card}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input ref={phoneRef} style={S.input} placeholder="01XXXXXXXXX" value={form.phone}
                onChange={e => handlePhoneChange(e.target.value)} autoFocus />
            ...
            <div style={S.row2}>
              <div>
                ...
                <input style={S.input} placeholder="اسم العميل الكامل" value={form.customerName}
                  onChange={e => setForm(f => ({ ...f, customerName: e.target.value }))} />
              </div>
              <div>
                <label style={S.label}>👤 اسم السيلز *</label>
                <input style={S.input} placeholder="اسم موظف المبيعات" value={form.sales}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <textarea style={S.textarea} placeholder="مثال: العميل مهتم بوحدات 3 غرف، محتاج تقسيط على 10 سنين..." value={form.feedback}
                onChange={e => setForm(f => ({ ...f, feedback: e.target.value }))} />
            </div>

            <button style={{ ...S.btn, opacity: duplicateAlert ? .4 : 1 }} onClick={handleSubmit} disabled={!!duplicateAlert}>
              إضافة العميل ✓
            </button>
          </div>
        )}
        ...
        {tab === "list" && (
          <>
            <input style={S.searchBar} placeholder="🔍 ابحث باسم العميل أو رقم التليفون أو السيلز أو المشروع..."
              ...
              <div key={l.id} style={S.leadCard}>
                <div style={S.leadHeader}>
                  ...
                  <span style={S.badge(l.status)}>
                    <span style={S.dot(l.status)} />{l.status}
                  </span>
                </div>

                <div style={S.infoRow}>
                  <div style={S.infoItem}>
                    <span style={S.infoLabel}>👤 السيلز</span>
                    <span style={S.infoValue}>{l.sales}</span>
                  </div>
                  <div style={S.infoItem}>
                    <span style={S.infoLabel}>🏢 المشروع</span>
                    <span style={S.infoValue}>{l.project}</span>
                  </div>
                  {l.updatedAt !== l.createdAt && (
                    <div style={S.infoItem}>
                      <span style={S.infoLabel}>🔄 آخر تحديث</span>
                      <span style={S.infoValue}>{timeAgo(l.updatedAt)}</span>
                    </div>
                  )}
                </div>
                ...
                <div style={S.actions}>
                  ...
                  <a href={`https://wa.me/2${l.phone}`} target="_blank" rel="noreferrer">
                    <button style={S.btnSm("#14532d")}>💬 واتساب</button>
                  </a>
                  <a href={`tel:${l.phone}`}>
                    <button style={S.btnSm("#1e2d3a")}>📞 اتصل</button>
                  </a>
          ...
          <>
            <div style={S.statGrid}>
              <div style={S.statCard("#3b82f6")}>
                ...
                <div style={{ ...S.statVal, color: "#60a5fa" }}>{totalLeads}</div>
                <div style={S.statLbl}>إجمالي العملاء</div>
              </div>
              <div style={S.statCard("#10b981")}>
                <div style={{ fontSize: 28, marginBottom: 6 }}>🏆</div>
                <div style={{ ...S.statVal, color: "#34d399" }}>{leads.filter(l => l.status === "تم البيع").length}</div>
                <div style={S.statLbl}>صفقات مغلقة</div>
              </div>
              <div style={S.statCard("#f59e0b")}>
                <div style={{ fontSize: 28, marginBottom: 6 }}>🔥</div>
                <div style={{ ...S.statVal, color: "#fbbf24" }}>{leads.filter(l => l.status === "مهتم" || l.status === "زيارة مجدولة").length}</div>
                <div style={S.statLbl}>ليدز ساخنة</div>
              </div>
            </div>
            ...
            <div style={S.card}>
              ...
              {topSales.map(([name, count], i) => (
                <div key={name} style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
                  <div style={{ width: 30, height: 30, borderRadius: "50%", background: i === 0 ? "#fbbf2420" : "#1a2540", color: i === 0 ? "#fbbf24" : "#94a3b8", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 14, flexShrink: 0 }}>
                    {i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : i + 1}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      <span style={{ fontSize: 13, fontWeight: 700 }}>{name}</span>
                      <span style={{ fontSize: 13, color: "#60a5fa", fontWeight: 700 }}>{count} عميل</span>
                    </div>
                    <div style={{ height: 6, background: "#1a2540", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${(count / topSales[0][1]) * 100}%`, background: i === 0 ? "#f59e0b" : "#3b82f6", borderRadius: 3 }} />
                    </div>
                  </div>
                </div>
              ))}
                ...
                {STATUS_OPTIONS.map(s => (
                  <div key={s} style={{ background: STATUS_STYLE[s].bg, borderRadius: 10, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ width: 8, height: 8, borderRadius: "50%", background: STATUS_STYLE[s].dot }} />
                      <span style={{ fontSize: 13, color: STATUS_STYLE[s].color, fontWeight: 600 }}>{s}</span>
                    </div>
      ...
      {editId && (
        <div style={S.modalOverlay} onClick={() => setEditId(null)}>
          <div style={S.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={S.label}>🧑 اسم العميل</label>
              <input style={S.input} placeholder="اسم العميل" value={editForm.customerName || ""}
                onChange={e => setEditForm(f => ({ ...f, customerName: e.target.value }))} />
            </div>

            <div style={{ marginBottom: 16 }}>
              <label style={S.label}>🏢 المشروع</label>
              <select style={S.select} value={editForm.project} onChange={e => setEditForm(f => ({ ...f, project: e.target.value }))}>
                {PROJECTS.map(p => <option key={p}>{p}</option>)}
              </select>
            </div>
              ...
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                {STATUS_OPTIONS.map(s => (
                  <button key={s} onClick={() => setEditForm(f => ({ ...f, status: s }))}
                    style={{ padding: "6px 12px", borderRadius: 20, border: `1px solid ${editForm.status === s ? STATUS_STYLE[s].dot : "#1a2540"}`, background: editForm.status === s ? STATUS_STYLE[s].bg : "transparent", color: editForm.status === s ? STATUS_STYLE[s].color : "#64748b", fontSize: 12, cursor: "pointer" }}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={S.label}>💬 الفيدباك</label>
              <textarea style={S.textarea} value={editForm.feedback || ""}
